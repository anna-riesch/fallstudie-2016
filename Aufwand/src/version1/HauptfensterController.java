...
public class HauptfensterController {
	...
	private void initialize() throws SQLException {
		...
		tbl_projektTabelle.getSelectionModel().selectedItemProperty().addListener(
	            (observable, oldValue, newValue) -> tabelle_geklickt(tbl_projektTabelle.getSelectionModel().getSelectedItem()));
	}
	...
	public void text_projektname_geklickt(ActionEvent event) throws SQLException {
		...
		int anzahl = db.projektname_aendern(tbl_projektTabelle.getSelectionModel().getSelectedItem().getProjektid(), text_projektname.getText());
		System.out.println(anzahl + " Datensätze geändert!");
		projektData.get(tbl_projektTabelle.getSelectionModel().getSelectedIndex()).setProjektname(text_projektname.getText());
	...
	public void button_kompetenzen_click(ActionEvent event) throws SQLException {
		...
		if (tbl_projektTabelle.getSelectionModel().getSelectedItem() != null);{
			...
			try {
				new OpenKompetenzStage(tbl_projektTabelle.getSelectionModel().getSelectedItem().getProjektid(), tbl_projektTabelle.getSelectionModel().getSelectedItem().getProjektname());
			} catch (Exception e) {
